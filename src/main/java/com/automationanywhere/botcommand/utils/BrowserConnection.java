...
public class BrowserConnection implements CloseableSessionObject {
  ...
  private static List<String> prepareDriverArguments(boolean headless, String profilePath,
    ...
    List<String> mergedArguments = new ArrayList<>(
        userArguments != null ? userArguments : new ArrayList<>());
    mergedArguments.addAll(defaultArguments.stream()
        .filter(arg -> mergedArguments.stream().noneMatch(a -> a.startsWith(arg)))
        .collect(Collectors.toList()));

    mergedArguments.removeIf(a -> a.equals(DriverArguments.HEADLESS.getArgument()) || a.startsWith(
        DriverArguments.USER_DATA_DIR.getArgument()));
