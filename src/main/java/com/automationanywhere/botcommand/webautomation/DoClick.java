...
public class DoClick {
    ...
    public void action(
            ...
            @Idx(index = "4", type = AttributeType.SELECT, options = {
                    @Idx.Option(index = "4.1", pkg = @Pkg(label = "Click", value = BrowserUtils.Click)),
                    @Idx.Option(index = "4.2", pkg = @Pkg(label = "Right Click (No JS)", value = BrowserUtils.RightClick)),
                    @Idx.Option(index = "4.3", pkg = @Pkg(label = "Double Click (No JS)", value = BrowserUtils.DoubleClick))})
            @Pkg(label = "Click Type", default_value = BrowserUtils.Click, default_value_type = STRING) @NotEmpty String clicktype,
        ...
        try {
            ...
            utils.doClick(driver, search, type, clicktype, library, timeout.intValue(), attribute);
        } catch (Exception e) {
