...
@CommandPkg(label = "Move to", name = "movetoelement",
        description = "Move to an Element",
        node_label = "element {{search}} for session {{session}}", icon = "pkg.svg", comment = true, group_label = "Click", text_color = "#2F4F4F", background_color = "#2F4F4F")
...
public class MoveTo {
    ...
    public void action(
        ...
        try {
            ...
            utils.moveTo(driver, search, type, library, timeout.intValue(), attribute);
        } catch (Exception e) {
