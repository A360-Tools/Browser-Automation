...
public class SendKeys {
    ...
    public void action(
            ...
            @Pkg(label = "Keys", default_value = "KEYS", default_value_type = STRING)
            @SelectModes @NotEmpty String inputtype,
            @Idx(index = "4.1.1", type = AttributeType.KEYPRESS) @Pkg(label = "Keys", default_value_type = DataType.FILE) @NotEmpty String keys,
            @Idx(index = "4.2.1", type = CREDENTIAL) @Pkg(label = "Credential", default_value_type = STRING) @NotEmpty SecureString keyscredential,
        ...
        try {
            ...
            utils.sendKeys(driver, search, type, keyString, library, timeout.intValue(), attribute);
        } catch (Exception e) {
