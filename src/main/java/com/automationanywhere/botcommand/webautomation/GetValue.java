...
@CommandPkg(label = "Get Value", name = "getvalueelement",
    ...
    node_label = "of element {{search}} and assign to {{returnTo}} for session {{session}}", icon = "pkg.svg",
    comment = true, group_label = "Get", text_color = "#2F4F4F", background_color = "#2F4F4F",
...
public class GetValue {
  ...
  public static StringValue action(
      ...
      @Idx(index = "4", type = AttributeType.NUMBER) @Pkg(label = "Timeout (Seconds)", description =
          "No wait " +
    ...
    try {
      ...
      throw new BotCommandException("Get value failed" + ": Element is no longer attached to the DOM. Selector: " + search + " (" + type + "). The page may have refreshed or the element was removed.");
    } catch (ElementNotInteractableException e) {
      throw new BotCommandException("Get value failed" + ": Element found but not interactable. Selector: " + search + " (" + type + "). It may be hidden, disabled, or covered.");
    } catch (TimeoutException e) {
      throw new BotCommandException("Get value failed" + ": Timed out waiting for element. Selector: " + search + " (" + type + "). " + e.getMessage());
    } catch (InvalidSelectorException e) {
      throw new BotCommandException("Get value failed" + ": Invalid selector. Selector: " + search + " (" + type + "). " + e.getMessage());
    } catch (Exception e) {
      throw new BotCommandException(
          "Get value failed " + search + " " + type + " : " + e.getMessage());
    }
