...
@BotCommand
@CommandPkg(label = "Select Frame", name = "selectframe",
        description = "Select Frame",
        node_label = "{{search}} for session {{session}}", icon = "pkg.svg", group_label = "Frame", comment = true, text_color = "#2F4F4F", background_color = "#2F4F4F")
...
public class SelectFrame {
    ...
    public void action(
            ...
            @Idx(index = "2", type = AttributeType.TEXTAREA) @Pkg(label = "Frame Search", description = "Should match the type", default_value_type = STRING) @NotEmpty String search,
        ...
        try {
            ...
            utils.switchToFrame(driver, search, type, library, timeout.intValue(), attribute);
        } catch (Exception e) {
