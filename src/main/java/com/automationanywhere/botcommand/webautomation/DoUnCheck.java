...
@CommandPkg(label = "Uncheck", name = "uncheckelement",
    description = "Unselect an Element",
    node_label = "{{search}} for session {{session}}", icon = "pkg.svg", comment = true, group_label = "Set",
...
public class DoUnCheck {
  ...
  public static void action(
    ...
    try {
      ...
      throw new BotCommandException("Uncheck failed" + ": Element is no longer attached to the DOM. Selector: " + search + " (" + type + "). The page may have refreshed or the element was removed.");
    } catch (ElementNotInteractableException e) {
      throw new BotCommandException("Uncheck failed" + ": Element found but not interactable. Selector: " + search + " (" + type + "). It may be hidden, disabled, or covered.");
    } catch (TimeoutException e) {
      throw new BotCommandException("Uncheck failed" + ": Timed out waiting for element. Selector: " + search + " (" + type + "). " + e.getMessage());
    } catch (InvalidSelectorException e) {
      throw new BotCommandException("Uncheck failed" + ": Invalid selector. Selector: " + search + " (" + type + "). " + e.getMessage());
    } catch (Exception e) {
      throw new BotCommandException(
          "uncheck element failed " + search + " " + type + " : " + e.getMessage());
    }
